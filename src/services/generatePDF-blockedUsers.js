...
export const generatePDFBlockedUsers = async (usersData) => {
  ...
  // Ancho de columna personalizado para el campo de correo y teléfono
  const emailColumnWidth = 200; // Ajusta el ancho según tus necesidades
  const phoneColumnWidth = 200; // Ajusta el ancho según tus necesidades

  // Declarar la variable headerColumns antes de su uso
  const headerText = "Nombre, Apellido, Usuario, Correo, Teléfono";
  ...
  for (let pageNum = 0; pageNum < numPages; pageNum++) {
    ...
    page.drawText(`Lista de Usuarios Bloqueados - Página ${pageNum + 1}`, {
      x: 50,
      y: yOffset,
      size: 16,
      color: rgb(0, 0, 0),
    ...
    for (let j = 0; j < headerColumns.length; j++) {
      ...
      } else if (columnText === "Teléfono") {
        // Usar el ancho personalizado para el campo de teléfono
        page.drawText(columnText, {
          x,
          y: tableY - 12,
          size: 12,
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      const columns = [
        ...
        `@${user.username}`,
        user.email,
        user.phone, // Agregar el campo de teléfono
      ];
