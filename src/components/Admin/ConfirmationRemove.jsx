...
function ConfirmationDesblock({
  ...
  return (
    ...
    >
      <p className="mb-2">
        ¿Estás seguro de que deseas removerle los permisos de Administrador a{" "}
        <strong>{selectedUser.email}</strong>?
      </p>
      <div className="flex justify-center text-sm">
        <button
          onClick={() => handleConfirmRemove(selectedUser.id)}
          className={`px-4 py-2 rounded-lg bg-red-500 text-white font-bold ${
            loading ? "opacity-50 cursor-not-allowed" : "hover:bg-red-600"
          }`}
          disabled={loading}
        >
          {loading ? "Removiendo permisos..." : "Confirmar"}
        </button>
        <button
...
export default ConfirmationDesblock;
